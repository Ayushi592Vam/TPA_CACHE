...
Renders the Claim-Level Duplicate Detection panel inside the middle column.

Shows:
  - A banner when the current claim ID was seen in a previous upload
  - A before/after diff table for every changed field
  - A summary of unchanged fields
  - A "Clear history for this claim" button
"""
...
def render_claim_dup_panel(claim_id: str, dup_results: dict) -> None:
    """
    Call this just below the claim header in claim_panel.py.
    Only renders something if the claim is a duplicate.
    """
    ...
    old_fields      = result.get("old_fields", {})
    ...
    if st.session_state[_diff_key]:
        ...
        st.markdown(
            ...
            "border:1px solid #2a2a45;border-top:none;margin-bottom:10px;'></div>",
            unsafe_allow_html=True,
        )
    ...
    if st.button(
        ...
        help="Removes this claim from the duplicate store so it won't be flagged next time",
    ):
        store = _load_claim_dup_store()
        if claim_id in store:
            del store[claim_id]
            _save_claim_dup_store(store)
            st.success(f"Cleared history for claim {claim_id}")
            st.rerun()
